...
/**
 * GuidedInput — Beginner learning phase.
 * Like FirstLetterInput, but unrevealed words show the actual word text
 * in a dimmed/muted style instead of underscores. User types the first
 * letter of each word to advance through the verse.
 */
export default function GuidedInput({ verseText, onComplete }: GuidedInputProps) {
  ...
  useEffect(() => {
    const el = document.getElementById(`guided-word-${currentIdx}`);
    if (el) {
  ...
  return (
    <div ref={containerRef} className="space-y-3">
      ...
      {/* Word tokens display — dimmed text for unrevealed words */}
      <div
      ...
      >
        {tokens.map((token, idx) => {
          ...
          return (
            <motion.span
              ...
              className={`font-serif text-base transition-all duration-200 rounded-lg px-1.5 py-0.5 ${
                isRevealed
                  ...
                  : isCurrent
                  ? wrongFlash
                    ? 'bg-warmBrown-200 dark:bg-warmBrown-600/60 text-warmBrown-400 dark:text-warmBrown-400'
                    : 'text-warmBrown-400 dark:text-parchment-500 ring-2 ring-olive-400 dark:ring-olive-400 bg-olive-50 dark:bg-olive-900/40'
                  : 'text-parchment-400/60 dark:text-warmBrown-600'
              }`}
              animate={
                isRevealed
            ...
            >
              {/* Always show the actual word text — dimmed when unrevealed */}
              {token.display}
            </motion.span>
          );
        })}
      ...
      <div className="flex items-center justify-between">
        <p className="text-xs text-warmBrown-400 dark:text-parchment-400">
          Read along and type the first letter of each word · {tokens.filter((t) => t.revealed).length}/{tokens.length} words
        </p>
        {!completed && (
          <button
            onClick={() => {
